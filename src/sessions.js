...
const initializeEvents = (client, sessionId) => {
  ...
  checkIfEventisEnabled('vote_update')
    .then(_ => {
      client.on('vote_update', (message) => {
        triggerWebhook(sessionWebhook, sessionId, 'vote_update', { message })
      })
    })
  ...
  checkIfEventisEnabled('ready')
    .then(_ => {
      client.on('ready', () => {
        triggerWebhook(sessionWebhook, sessionId, 'ready', { workerId: process.env.WORKER_ID })
      })
    })
